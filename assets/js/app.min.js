...
showFiles()}
function showFiles(){console.log(files);var newFileList=new DataTransfer();files.forEach(function(file){newFileList.items.add(file)});inputFileElement.files=newFileList.files;if(files.length>0){KB.dom(dropzoneElement).empty().add(buildFileListElement())}else{KB.dom(dropzoneElement).empty().add(buildInnerDropzoneElement())}
checkMaxSize()}
...
function onLoad(){console.log(options.files);if(options.files.length!==0){console.log('files',options.files)}
if(options.screenshot){var data='data:image/png;base64,'+options.screenshot;createImage(data)}}
