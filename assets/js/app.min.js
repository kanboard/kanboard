...
if(isOversize){KB.trigger('modal.disable');if(!message){KB.dom(containerElement).add(KB.dom('div').attr('id','message-container').attr('class','alert alert-error').text(options.labelOversize).build())}}else{KB.trigger('modal.enable');if(message){message.remove()}}}
