...
function checkMaxSize(){var isOversize=!1;var message=KB.find('#message-container');if(totalSize>options.maxSize){isOversize=!0}
if(isOversize){KB.trigger('modal.disable');if(!message){KB.dom(containerElement).add(KB.dom('div').attr('id','message-container').attr('class','alert alert-error').text(options.labelOversize).build())}}else{KB.trigger('modal.enable');message.remove()}}
...
showFiles()}
function showFiles(){if(files.length>0){KB.dom(dropzoneElement).empty().add(buildFileListElement())}else{KB.dom(dropzoneElement).empty().add(buildInnerDropzoneElement())}
checkMaxSize()}
...
function buildFileListItem(index){var progressElement=KB.dom('progress').attr('id','file-progress-'+index).attr('value',0).build();var deleteElement=KB.dom('span').attr('id','file-delete-'+index).html('<a href="#"><i class="fa fa-trash fa-fw"></i></a>').on('click',function(){totalSize-=files[index].size;files.splice(index,1);KB.find('#file-item-'+index).remove();showFiles()}).build();var itemElement=KB.dom('li').attr('id','file-item-'+index).add(deleteElement).add(progressElement).text(' '+files[index].name+' ');return itemElement.build()}
