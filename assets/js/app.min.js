...
showFiles()}
function showFiles(){var newFileList=new DataTransfer();for(var file of files)newFileList.items.add(file);inputFileElement.files=newFileList.files;if(files.length>0){KB.dom(dropzoneElement).empty().add(buildFileListElement())}else{KB.dom(dropzoneElement).empty().add(buildInnerDropzoneElement())}
checkMaxSize()}
