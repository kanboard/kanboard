...
- Your reverse proxy authenticates the user and adds a HTTP header containing its login to the request. The default header name or how to specify it should be in its documentation, however:
...
- Kanboard retrieves the value of the specified HTTP header, and:
  - if the user does not exist yet, creates it (it also check if it is the default admin).
...
This is not in the scope of this documentation. You only have to check that the user login is sent by the reverse proxy using a HTTP header (and find which one).
...
// defined('REVERSE_PROXY_USER_HEADER', 'REMOTE_USER);

// The default Kanboard admin for your organization.
// Since everything should be filtered by the reverse proxy, you should want to have a bootstrap admin user
