...
<?php else: ?>
    <ul>
        ...
        <li>
            ...
            <?= $this->url->link(t('Group by date'), 'app', 'notifications', array('user_id' => $user['id'], 'group_by' => 'date_creation'), false, '') ?>
        </li>
        <li>
            <i class="fa fa-hashtag"></i>
            <?= $this->url->link(t('Group by task'), 'app', 'notifications', array('user_id' => $user['id'], 'group_by' => 'task_id'), false, '') ?>
        </li>
    </ul>
</div>
    ...
    <table class="table-fixed table-small">
        ...
        <?php foreach ($notifications as $group_by => $grouped_notification): ?>
        <tr>
            <td colspan="4"><?= ($group_by > 1420070400) ? $this->dt->date($group_by) : "#" . (($group_by > 0) ? $group_by : "") ?></td>
        </tr>
            <?php foreach ($grouped_notification as $notification): ?>
            <tr>
                <td></td>
                <td>
